...
async function showSuggestions(city) {
    ...
    const url = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city)}&count=10&language=da&format=json`; try {
        const response = await fetch(url);
        const data = await response.json();
        ...
        // Only real cities, not countries or regions, and not "Europa"
        const validFeatureCodes = [
         ...
         const places = data.results.filter(function(place) {
           return (
               validFeatureCodes.includes(place.feature_code) && place.name.toLowerCase() !== "europa"
            );
    });

        // Maks. 5 forslag
        places.slice(0, 5).forEach(
            function(place) {
                ...
                button.classList.add("search__suggestion");


                button.innerHTML = `<strong>${place.name}</strong>
...
async function getWeather(latitude, longitude) {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,sunrise,sunset&timezone=auto&forecast_days=4`;
    const response = await fetch(url);

    if (!response.ok) {
        throw new Error(
            "Kunne ikke hente vejret."
        );
...
function getTimeOfDay(weather) {
    ...
    if (now >= sunriseStart && now < sunrise) {
        return "sunrise";
    }


    if (now >= sunset && now <= sunsetEnd) {
        return "sunset";
    }
...
// CHANGE BACKGROUND // 
function changeBackground(weatherCode, weather) {
    ...
    // Night
        ...
        timeOfDay === "night"){
        if ( weatherCode === 0 || weatherCode === 1) {background ="clear-night";
        }

        else if (
            weatherCode === 2 || weatherCode === 3 || weatherCode === 45 || weatherCode === 48) {background ="cloudy-night";
        }

        else if (
            weatherCode >= 71 && weatherCode <= 75) {background = "snow";
        }

        else if (
            weatherCode >= 95) {background = "storm";
        }
    ...
    // Day
        ...
        background = weatherDescriptions[weatherCode]?.background || "cloudy";
    }
...
function displayWeather(
    ...
    const weatherInfo =
        weatherDescriptions[current.weather_code] || {
            description: "Ukendt vejr", icon: "🌡️"
        };

    cityName.textContent = city.name;
    countryName.textContent = city.country || "Ukendt land";
    temperature.textContent = `${Math.round(current.temperature_2m)}°`;
    description.textContent = weatherInfo.description;
    weatherIcon.textContent = weatherInfo.icon;
    humidity.textContent = `${current.relative_humidity_2m}%`;
    wind.textContent = `${Math.round(current.wind_speed_10m)} km/t`;
...
function displayForecast(weather) {
    ...
    for (
        ...
        const weatherInfo = weatherDescriptions[codes[i]] || {icon: "🌡️"};
        const forecastCard = document.createElement("article");


        forecastCard.classList.add("forecast__day");

        forecastCard.innerHTML = `
            <p class="forecast__day-name">${dayName}</p>
            <p class="forecast__icon">${weatherInfo.icon}</p>
            <p class="forecast__temperature">${Math.round(temperatures[i])}°</p>
        `;
...
weatherForm.addEventListener("submit", function(event) {
        ...
        if (!city) {
            errorMessage.textContent = "Skriv venligst en by.";
            return;
        }
